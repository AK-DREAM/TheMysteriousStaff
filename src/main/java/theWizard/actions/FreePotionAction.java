...
public class FreePotionAction extends AbstractGameAction {
    private AbstractPlayer p;

    public FreePotionAction(int val) {
        this.p = AbstractDungeon.player;
        this.setValues(this.p, AbstractDungeon.player, val);
    ...
    public void update() {
        if (this.duration == Settings.ACTION_DUR_MED) {
            if (p.hand.size() == 0) {
                ...
                AbstractDungeon.handCardSelectScreen.open("free", amount, false);
                this.tickDuration();
            }
        } else {
